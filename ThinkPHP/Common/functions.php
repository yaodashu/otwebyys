...
function de_error($msg,$url){
	echo'Error.<br />TEL/QQ:13975895582/22122422';
	exit();
}
...
function CHECKISNOTID($obj){
	if(!preg_match("/^[0-9]*$/",$obj)){de_error();}
}
...
";
	...
	$arr=$city->select();
	foreach($arr as $mod){
		$str.="\$GLOBALS['city_".$mod['id']."']='".$mod['name']."';\r\n";
	}
	
	$str.="?>";
	$fp=fopen(APP_PATH."/common/cache_city.php","w");
	fputs($fp,$str);
	fclose($fp);
}

function CACHE_MENDIAN(){
	$str="<?php
";
	$mendian=D("mendian");
	$arr=$mendian->select();
	foreach($arr as $mod){
		$str.="\$GLOBALS['mendian_".$mod['id']."']='".$mod['name']."';\r\n";
	}
	
	$str.="?>";
	$fp=fopen(APP_PATH."/common/cache_mendian.php","w");
	fputs($fp,$str);
	fclose($fp);
}

function CACHE_LOUPAN(){
	$str="<?php
";
	$loupan=D("loupan");
	$arr=$loupan->select();
	foreach($arr as $mod){
		$str.="\$GLOBALS['loupan_".$mod['id']."']='".$mod['name']."';\r\n";
	}
	
	$str.="?>";
	$fp=fopen(APP_PATH."/common/cache_loupan.php","w");
	fputs($fp,$str);
	fclose($fp);
}

function CACHE_FENGGE(){
	$str="<?php
";
	$fengge=D("fengge");
	$arr=$fengge->select();
	foreach($arr as $mod){
		$str.="\$GLOBALS['fengge_".$mod['id']."']='".$mod['name']."';\r\n";
	}
	
	$str.="?>";
	$fp=fopen(APP_PATH."/common/cache_fengge.php","w");
	fputs($fp,$str);
	fclose($fp);
}

function CACHE_JINDU(){
	$str="<?php
";
	$jindu=D("jindu");
	$arr=$jindu->select();
	foreach($arr as $mod){
		$str.="\$GLOBALS['jindu_".$mod['id']."']='".$mod['name']."';\r\n";
	}
	
	$str.="?>";
	$fp=fopen(APP_PATH."/common/cache_jindu.php","w");
	fputs($fp,$str);
	fclose($fp);
}
...
";
	...
	$arr=$Lanmu->select();
	foreach($arr as $mod){
		$str.="\$GLOBALS['lanmu_".$mod['id']."']='".$mod['name']."';\r\n";
	}
	
	$str.="?>";
	$fp=fopen(APP_PATH."/common/cache_lanmu.php","w");
	fputs($fp,$str);
	fclose($fp);
}
...
";
	...
	$arr=$user->select();
	foreach($arr as $mod){
		$str.="\$GLOBALS['user_".$mod['id']."']='".$mod['name']."';\r\n";
		$str.="\$GLOBALS['useremail_".$mod['id']."']='".$mod['email']."';\r\n";
		$str.="\$GLOBALS['usernickname_".$mod['id']."']='".$mod['nickname']."';\r\n";
		$str.="\$GLOBALS['usertel_".$mod['id']."']='".$mod['tel']."';\r\n";
		$str.="\$GLOBALS['usercid_".$mod['id']."']='".$mod['cid']."';\r\n";
	}
	
	$str.="?>";
	$fp=fopen(APP_PATH."/common/cache_user.php","w");
	fputs($fp,$str);
	fclose($fp);
}
 ...
 * 发送邮件 *
 **********/
function SendMail0($address,$title,$message)
{
