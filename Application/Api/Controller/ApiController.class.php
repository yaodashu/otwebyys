...
abstract class ApiController extends Controller {
    ...
    public function _initialize() {
        // 读取站点信息,调用Common模块的API下ConfigApi中的lists方法,读取数据库中config表的内容并使用C方法加载
        ...
        // 定义API,加载User模块api下UserApi.class.php中定义的UserApi类
    ...
    protected function getCombinedUser($uid) {
        ...
        $map['uid'] = $uid;
        $map['status'] = 1;
        $home_info = D('User/Member')->where($map)->find();
        if(!$home_info) {
            $home_info = array();
        }
        //合并两者的信息
