...
class UserController extends ApiController
{
	...
	//管理员属性
        ...
        $avatar_url = getRootUrl() . $avatar_path;
	...
	$userinfo = array(
            ...
            'memberstatus' =>$user2['status'],
            'area' =>$user2['area'],
            'rank' =>$user2['rank'],
            'rightgroupid' =>$user2['rightgroupid'],
            'editlevel' =>$user2['editlevel'],
        );
	//判断用户分组
	if(intval($userinfo['rightgroupid'])>0){
	    $rights = M("right_group")->where("id=".$userinfo['rightgroupid'])->find();
        ...
        //只返回必要的详细资料
        $this->apiSuccess("获取成功", null, $userinfo);
    }

    public function setProfile($signature = null, $email = null, $mobile = null, $name = null, $sex = null, $birthday = null, $nickname = null)
    {
        $this->requireLogin();
        //获取用户编号
        $uid = $this->getUid();
        ...
        if ($home) {
            ...
            //TODO:此处无法通过$result进行判断更新是否成功,因为如果没有修改,$result一样返回0,只能在js中做好验证
            //if (!$result) {
