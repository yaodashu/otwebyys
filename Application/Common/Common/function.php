...
function get_sql_colorbase(){
    return " select DISTINCT a.id,a.color_name,b.path from jwu_colorbase a,jwu_picture b where a.id_picture= b.id ORDER BY color_name ";
}

function get_sql_store(){
    return "select id,StoreName,jwu_store.id_Area,jwu_area.AreaName,Address,nickname,id_photo,store_js,time_create from jwu_store LEFT JOIN jwu_area ON jwu_store.id_Area=jwu_area.AreaID ORDER BY sortid desc,score DESC ";
}
...
function is_ie()
{
 ...
 /**
     ...
     * @return bool
     */
    function _checkError($error_code,$_max_size,$_file_type,$_error_code)
    {    
        if ($error_code > 0) {
            //echo '<script>alert("'.$_error_code[$error_code].'");</script>';
            return false;
        }
        return true;
    }
     ...
     * @return bool
     */
     function _checkMaxSize($size,$_max_size,$_file_type,$_error_code)
    {
        if ($size > $_max_size) {
     ...
     * @return bool
     */
     function _checkFileType($type,$_max_size,$_file_type,$_error_code)
    {
        if ($type['extension'] == $_file_type || strpos($_file_type,$type['extension'])>=0) {
	}else{
     ...
     * @return bool
     */
     function _chekFileIsExists($filename)
    {
