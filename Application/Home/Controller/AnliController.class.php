...
//	    $map['mianji']= array("lt",$this->mianjis[$mianji]['max']);	    
	...
	if($keyword){
	    ...
	    $lists_anli1=D("AnliView")->where("document.status=1 and (document.title like '%{$keyword}%' or document_anli.loupan like '%{$keyword}%' or document_shejishi.nickname like '%{$keyword}%' )")->order("level desc,document.id desc")->limit($start.",".$psize)->select(); 
	}else{
	    $lists_anli1=D("AnliView")->where($map)->order("level desc,document.id desc")->limit($start.",".$psize)->select();  
	}
	//var_dump($lists_anli1);
	 
	$strs = "";
	foreach ($lists_anli1 as $k=>$v){
	    $strs =(empty($strs))?$v['shejishi']:($strs.",".$v['shejishi']);
	}  
	if($strs){
	    //获取设计师列表 
	    $maps['status']=1; 
	    $lists_shejishi1=M("document_shejishi")->where("sno in ({$strs})")->select();  
	    foreach ($lists_anli1 as $k=>$v){
		foreach ($lists_shejishi1 as $k1=>$v1){
		    if($v['shejishi']==$v1['sno']){
			$lists_anli1[$k]['sid']=$v1['id'];
		    }
		}
	    }
	}
...
//	echo microtime();
	...
	if(C('PC_MOBILE_SPLIT')==1){
	    $domain = trim(C("WEBSITE_URL_PC_ONLINE"));
	}
	...
	//echo $details[0]['headerpic'].$anli_data['picpath'];
//	$document_data = D("Document")->where(array("id"=>$aid))->find();
	...
	if($anli_data['shejishi']){
	    ...
	    $anli_nums = M()->query("select count(id) count from ot_document_anli where shejishi={$anli_data['shejishi']}");
	    //var_dump($anli_nums);
	    $shejishi_view['num_zuoping'] = $shejishi_view['num_zuoping'] + intval($anli_nums[0]['count']);
	}	
	$_sql = "select a.id,p.path detailpicpath,p1.path headerpic from ot_document_anli a LEFT JOIN ot_picture p on a.detailpic=p.id,ot_document_shejishi s LEFT JOIN ot_picture p1 on s.headerpic=p1.id where a.shejishi=s.sno and a.id in ({$aid})";
	$details = M()->query($_sql);
	$anli_data['detailpicpath'] = $details[0]['detailpicpath'];
	$shejishi_view['picpath'] = ($details[0]['headerpic'])?$details[0]['headerpic']:$shejishi_view['picpath'];
	...
	$anlis_view = D("AnliView")->where(array("status"=>1))->order("level desc")->limit("3")->select();
	$this->assign('anlis_view',$anlis_view);//  
	
	//var_dump($shejishi_view);
