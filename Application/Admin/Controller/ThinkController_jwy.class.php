...
class ThinkController extends AdminController {
    ...
    public function del_mem($model = null, $ids=null){
        if($model==null){
            $model    =   I('request.model');        
        }
        ...
        $Model = M(get_table_name($model['id']));
        $map_mem = array('uid' => array('in', $ids) );
        
        $Model_U = M('ucenter_member');
        $map = array('id' => array('in', $ids) );
        if($Model->where($map_mem)->delete() && $Model_U->where($map)->delete()){
            $this->success('删除成功');
        } else {
            $this->error('删除失败！');
        }
