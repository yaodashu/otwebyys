...
//	}
	...
	$start = ($pindex-1)*$psize;	
	//查询设楼盘案例
	if($rzloupan_view['dtitle']){
	    
	    $count=D("AnliView")->where(array("document.status=1 and (rzloupan ='".trim($rzloupan_view['dtitle'])."')"))->count();
	    
	    $page =new \Think\Page($count,$psize);
	    $start = $page->firstRow;
	    $psize = $page->listRows; 
	     
	    $anlis_view = D("AnliView")->where(array("document.status=1 and (rzloupan ='".$rzloupan_view['dtitle']."')"))->order("level desc")->limit($start.",".$psize)->select();
