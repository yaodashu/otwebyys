...
//	echo $encode;
	...
	$lists_data1=D("ArticleView")->where($map)->order("level desc")->limit($start.",".$psize)->select(); 
...
//	$page->setConfig('theme', '%FIRST% %UP_PAGE% %LINK_PAGE% %DOWN_PAGE% %END% %HEADER%');
	...
	$aid=$_REQUEST['aid'];
	if(empty($aid)){
	    redirect("/index.php/home/news/index");
	}
...
//	
	//推荐案例 {
	 $anlis_view = D("AnliView")->where(array("status"=>"1"))->order("rand()")->limit("3")->select(); 
	 $anli_view = $anlis_view[0];
	 if($anli_view){
	    //推荐案例设计师 {
	     $shejishi_view = D("ShejishiView")->where(array("status"=>"1","sno"=>$anli_view['shejishi']))->limit("1")->find(); 
	 }	
